			...
			{visibleItems.map((item) => (
				<li key={item.url}>
					<a
						...
						className="transition-200 block max-w-[200px] truncate text-ellipsis whitespace-nowrap rounded-full p-2 text-black transition-colors hover:bg-slate-100 hover:text-black dark:text-white  dark:hover:bg-neutral-800 dark:hover:text-white"
					>
						{item.label}
					</a>
				</li>
			...
			<Container className="flex w-full items-center justify-between">
				...
				<div className="hidden items-center gap-5 lg:flex">
					<nav className="flex space-x-2 text-black dark:text-white">
						...
						<button
							onClick={toggleTheme}
							className="transition-200 block rounded-full p-2 transition-colors hover:bg-slate-100 hover:text-black dark:hover:bg-neutral-800 dark:hover:text-white md:p-3"
						>
							{theme === 'light' ? (
